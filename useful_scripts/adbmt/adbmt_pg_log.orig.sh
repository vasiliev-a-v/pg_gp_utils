...
func_parse_args() {  # arguments parsing
  ...
  while (( $# )); do
    case "$1" in
      ...
      -debug|--debug)           DEBUG="1";;
      *) func_prt_err "Неизвестная опция: $1"; func_print_usage; exit 1;;
    esac
    shift || true
  done
...
func_check_args() {  # проверить входные аргументы -------------------------- ##
  if [[ ! -n "$GPSEG_OPT" ]]; then
    func_prt_err "Опция -gpseg обязательна"
    exit 1
  fi
  if [[ ! "$free_spc" =~ ^([0-9]+)(\.[0-9]+)?$ ]]; then
    func_prt_err "-free-space должно быть числом"
    exit 1
  fi
  if ! func_validate_datetime_strict "$START_STR"; then
    func_prt_err "Некорректный start: $START_STR"
    exit 1
  fi
  if ! func_validate_datetime_strict "$END_STR"; then
    func_prt_err "Некорректный end: $END_STR"
    exit 1
  fi
  if [[ ! -r "$all_hosts" ]]; then
    func_prt_err "Не найден файл хостов кластера СУБД: $all_hosts"
    exit 1
  fi
}
...
func_parse_gpseg() {  ## Разбор -gpseg на ROLE и номер сегмента ------------- ##
  ...
  if [[ "$s" =~ ^-?[0-9]+$ ]]; then
    ...
    func_prt_err "Неверный -gpseg: $s (ожидается -1 | N | pN | mN)"
    exit 1
  fi

  if [[ "$ROLE" != "master" ]]; then
    [[ "$GPSEG" =~ ^[0-9]+$ ]] || \
      { func_prt_err "gpseg должен быть >=0"; exit 1; }
  else
    [[ "$GPSEG" == "-1" ]] || \
    { func_prt_err "Для master используйте -gpseg -1"; exit 1; }
  fi
}
...
func_debug_remote_probe() {  # быстрая проверка удалённо
  ((DEBUG == 1)) || return 0
  func_dbg "Пробую ls/стат на ${SEG_HOST}:${PG_LOG_PATH}"
