...
\set kill_hung_query_version 0.2
...
-- фильтрует из ps строку с нашим запущенным скриптом.
-- сохраняет в переменную psql_command.
-- Эти все действия для записи в лог-файл.
...
SELECT ''
  ...
  С помощью утилиты psql выполнить файл-скрипт kill_hung_query.
  Пример строки с командой:
  psql -d template1 -U gpadmin --set=sess_id=1234 --set=signal=SIGTERM \
    -f /home/gpadmin/arenadata_configs/kill_hung_query.sql

  Где опции в строке означают:
  -f - путь к текущему файлу-скрипту.
  Обратная косая черта ’\’ перед -f - просто перевод строки в оболочке.

  --set=sess_id - ID сессии распределённого SQL-запроса.
  Если опция --set=sess_id не указана, то скрипт выведет
  инструкцию по использованию скрипта (USAGE) и завершит работу.

  --set=signal  - тип посылаемого сигнала:
...
\qecho kill_hung_query_version: :kill_hung_query_version
